...
// KeyValueEditor
//
// Replaces all comma-separated "fieldName:label" textarea inputs throughout
// the settings panel. Renders a two-column table of text inputs (one row per
// pair) with Add Row and Remove buttons. Serialises back to the same
// "key:value,key:value" format the widget runtime already expects, so no
// runtime changes are needed for this component.
//
// Props:
//   value            - current comma-separated pairs string from config
//   onChange         - called with the new serialised string on every edit
//   keyPlaceholder   - column header / placeholder for the left (key) column
//   valuePlaceholder - column header / placeholder for the right (value) column
// ─────────────────────────────────────────────────────────────────────────────
    ...
    return raw.split(',')
        .map(pair => {
            ...
            return {
                ...
                val: pair.slice(colonIdx + 1).trim()
            }
        })
...
const KeyValueEditor: React.FC<KeyValueEditorProps> = ({
    ...
    // Initialise from the config value once on mount. Changes made via this
    // component are immediately serialised and written back to config via
    // onChange, so the prop and local state stay in sync without a loop.
    const [rows, setRows] = React.useState<KvRow[]>(() => parseKvString(value))

    const updateRows = (next: KvRow[]) => {
        setRows(next)
        onChange(serialiseKv(next))
    }

    const onCellChange = (idx: number, part: 'key' | 'val', text: string) => {
        const next = rows.map((r, i) => i === idx ? { ...r, [part]: text } : r)
        updateRows(next)
    }
    ...
    const kvRemoveBtnStyle: React.CSSProperties = {
        fontSize: '11px',
        color: '#cc0000',
        background: 'none',
        border: 'none',
    ...
    const kvAddBtnStyle: React.CSSProperties = {
        fontSize: '10px',
        padding: '3px 8px',
        backgroundColor: '#076FE5',
        color: 'white',
...
// ─────────────────────────────────────────────────────────────────────────────
// Setting component state interface
// ─────────────────────────────────────────────────────────────────────────────
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, State> {
    ...
    // Shared config updater for KeyValueEditor onChange callbacks
    onKvChange = (key: string) => (val: string) => {
        this.props.onSettingChange({
            id: this.props.id,
            config: this.props.config.set(key, val)
        })
    }

    // ── Target widget ID helpers ──────────────────────────────────────────────
    ...
    onAddListenChannel = () => {
        ...
        this.props.onSettingChange({
            id: this.props.id,
            config: this.props.config.set('listenChannels', [...this.getListenChannels(), newEntry])
        })
    ...
    onAddDispatchEvent = () => {
        ...
        this.props.onSettingChange({
            id: this.props.id,
            config: this.props.config.set('dispatchEvents', [...this.getDispatchEvents(), newEntry])
        })
    ...
    renderListenerCard = (entry: ListenChannelEntry, index: number) => (
        <div key={index} style={listenerCardStyle}>
            <div style={listenerCardHeaderStyle}>
                ...
                <button style={removeBtnStyle} onClick={() => this.onRemoveListenChannel(index)}>
                    Remove
                </button>
            </div>
    ...
    renderAddListenerForm = () => {
        ...
        return (
            <div style={{ border: '1px dashed #076FE5', padding: '8px 10px', marginBottom: '8px' }}>
                ...
                <SettingRow>
                    <input type='text' value={newListenerChannel}
                ...
                <SettingRow>
                    <select style={selectStyle} value={newListenerAction}
                        onChange={e => this.setState({ newListenerAction: e.target.value as 'refresh' | 'populate' })}>
                ...
                {newListenerAction === 'populate' && (
                    <>
                        ...
                        <SettingRow>
                            <input type='text' value={newListenerPopulateChannel}
                        ...
                        <SettingRow>
                            <input type='text' value={newListenerPopulateTargets}
    ...
    renderDispatchEventCard = (entry: DispatchEventEntry, index: number) => (
        <div key={index} style={listenerCardStyle}>
            <div style={listenerCardHeaderStyle}>
                <span style={listenerChannelLabelStyle}>{entry.channel}</span>
                <button style={removeBtnStyle} onClick={() => this.onRemoveDispatchEvent(index)}>
                    Remove
                </button>
            </div>
            <div style={{ fontSize: '11px', color: '#666', fontFamily: 'Arial' }}>
            ...
            <div style={{ border: '1px dashed #076FE5', padding: '8px 10px', marginBottom: '8px' }}>
                ...
                <SettingRow>
                    <input
                        ...
                        placeholder='e.g. ETSReviewFeatureSelected'
                        style={{
                            width: '100%', padding: '4px 6px', fontSize: '12px',
                            border: '1px solid #ccc', borderRadius: 0, fontFamily: 'Arial'
                        }}
                ...
                {/* Payload fields — KeyValueEditor replaces the old free-text textarea */}
                <SettingRow label='Payload fields (optional)' />
                <SettingRow>
                    <div style={hintStyle}>
                        One row per field to include from the clicked row in the payload.
        ...
        return (
            <div>
                ...
                <SettingSection title={fmt('sectionData', defaultI18nMessages.sectionData)}>
                    ...
                    {/* Column headers — KeyValueEditor replaces free-text textarea */}
                ...
                <SettingSection title={fmt('sectionDisplay', defaultI18nMessages.sectionDisplay)}>
                    ...
                    {/* No data message — improvement #10 */}
                ...
                {c.showRowActionButton && (
                    <SettingSection title={fmt('sectionPerRow', defaultI18nMessages.sectionPerRow)}>
                        ...
                        <SettingRow>
                            ...
                            <select style={selectStyle} value={c.rowActionMode || 'api'}
                        ...
                        {isApiMode && (
                            <>
                                ...
                                <SettingRow>
                                    <select style={selectStyle} value={c.rowActionHttpMethod || 'PUT'}
                                ...
                                <SettingRow>
                                    ...
                                    <select style={selectStyle} value={c.rowActionParamMode || 'query'}
                                ...
                                <SettingSection title='Post-action notification (optional)'>
                                    ...
                                    {postActionEnabled && (
                                        <>
                                            ...
                                            {/* Payload fields — KeyValueEditor */}
                                            <SettingRow label='Payload fields (optional)' />
                                            <SettingRow>
                                                <div style={hintStyle}>
                                                    One row per field from the clicked row to include in the payload.
                        ...
                        {isNavigateMode && (
                            <>
                                ...
                                <SettingRow>
                                    ...
                                    <select style={selectStyle} value={c.navigateTargetType || 'page'}
                                ...
                                <SettingRow>
                                    <div style={hintStyle}>
                                        ...
                                        {navigateTargetType === 'widget' && 'The widget ID of the widget to scroll to e.g. widget_75'}
                                    </div>
                                ...
                                {/* URL parameters — KeyValueEditor */}
                                <SettingRow label='URL parameters to pass on navigation' />
                                <SettingRow>
                                    <div style={hintStyle}>
                                        One row per parameter. Maps row data fields to URL parameters
                                        appended to the destination page URL.
                                    </div>
                                    <KeyValueEditor
                ...
                {c.showGlobalButton && (
                    <SettingSection title={fmt('sectionGlobal', defaultI18nMessages.sectionGlobal)}>
                        ...
                        <SettingRow label='Append username to global action URL'>
                            <Switch checked={c.use_username_for_global_action || false}
                                onChange={this.onBoolChange('use_username_for_global_action')} />
                        </SettingRow>

                        <SettingRow label='Include username in global action payload'>
                            <Switch checked={c.use_username_in_global_payload || false}
                                onChange={this.onBoolChange('use_username_in_global_payload')} />
                        </SettingRow>

                        {/* Minimum rows to activate — improvement #9 */}
                            ...
                            <TextInput
                                ...
                                onChange={evt => this.props.onSettingChange({
                                    ...
                                    config: this.props.config.set(
                                        'globalButtonMinRows',
                                        Math.max(1, parseInt(evt.currentTarget.value) || 2)
                                    )
                                })}
                        ...
                        <SettingSection title={fmt('sectionGlobalPayload', defaultI18nMessages.sectionGlobalPayload)}>
                            ...
                            {/* Fixed default values — KeyValueEditor (both modes) */}
